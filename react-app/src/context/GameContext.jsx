...
export const GameProvider = ({ children }) => {
  ...
  const [trumpCaller, setTrumpCaller] = useState(null)
  const [trumpCallerTeam, setTrumpCallerTeam] = useState(null)
  const [currentDealer, setCurrentDealer] = useState('south')
  ...
  const dealCards = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      Object.keys(newPlayers).forEach(pos => {
        newPlayers[pos].cards = []
        newPlayers[pos].tricks = 0
      })
  ...
  }, [currentDealer, players, stats])

  // Start trump selection
  const startTrumpSelection = (flipped) => {
  ...
  const processTrumpSelection = useCallback((player, round, passed) => {
    if (player === 'south') {
  ...
  const aiTrumpSelection = (player, round, passed) => {
    const playerName = playerSettings[player].name
    ...
    if (round === 1) {
      ...
      if (player === currentDealer && passed.length === 3) {
        // Stick the dealer
        ...
        return
      } else if (Math.random() < (aggressiveness + 0.2)) {
        const availableSuits = ['♠', '♥', '♦', '♣'].filter(s => s !== flippedCard.suit)
        const chosenSuit = availableSuits[Math.floor(Math.random() * availableSuits.length)]
        selectTrump(chosenSuit, player)
  ...
  // Next trump player
  const nextTrumpPlayer = (player, round, passed) => {
  ...
  // Select trump
  const selectTrump = (suit, player = trumpSelectionPlayer) => {
    setTrump(suit)
  ...
  const startPlay = () => {
    ...
    setTimeout(() => {
      ...
      if (playingAlone && nextPlayer === getPartner(alonePlayer)) {
  ...
  const playCard = (player, cardIndex) => {
    ...
    if (playingAlone && player === getPartner(alonePlayer)) {
  ...
  const nextPlayer = (player) => {
    ...
    let next = players[(currentIndex + 1) % 4]

    if (playingAlone && next === getPartner(alonePlayer)) {
  ...
  const isCardPlayable = (card, player) => {
